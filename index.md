# System Help and Educational Guides

Effective May 2025, MRETS Inc. has rebranded as CleanCounts. You are reading FAQs for the MRETS registry, which will continue to be operated under the MRETS service name inside the CleanCounts organization.

### [Enrolling in Multifactor Authentication (MFA)](https://mrets.github.io/Help/mfa_setup)
This article will walkthrough how to enroll in mutli factor authentication (MFA). 

### [Filtering and Downloading Tables](https://mrets.github.io/Help/filtering_tables)
This article will walkthrough how to filter and download tables within M-RETS.

<br>

## Certificates

### [Certificates Dashboard: An Introduction](https://mrets.github.io/Help/Certificates_Dashboard)
This article will provide an introduction to the Certificates Dashboard. 

### [Reserve Certificates Overview](https://mrets.github.io/Help/Reserve_Certificates_Overview)
This article will provide an overview on what a reserve is. 

### [Transferring Certificates](https://mrets.github.io/Help/certificates_transferring_certificates)
This article will walkthrough how to transfer a certificate(s).

### [Retiring Certificates](https://mrets.github.io/Help/certificates_retiring_certificates)
This article will walkthrough how to retire certificates. 

### [Certificate Retirement Types](https://mrets.github.io/Help/certificate_retirements)
This article will walkthrough how the retirement types in M-RETS.


<br>

## Transactions

### [How-To: Automatic Recurring Transfers (ARTs) and Irrevocable Transfers](https://mrets.github.io/Help/transactions_automatic_recurring_transfers)
This article will walkthrough how to create an automatic recurring transfer.

### [Withdrawing/Accepting/Rejecting Transfers](https://mrets.github.io/Help/transactions_withdrawing_accepting_rejecting_transfers)
This article will walkthrough how to withdraw, accept, and reject a transfer.

### [Exporting RECs](https://mrets.github.io/Help/certificates_exporting_certifcates)
This article will walkthrough how to export certificates to suppoprted registries. 

### [Mass Certificate Transfers](https://mrets.github.io/Help/transactions_mass_certificate_transfers)
This article will walkthrough how to use the mass certificate transfers fucntionality. 


<br>

## Hourly Data

### [View Hourly Data Reports](https://mrets.github.io/Help/hourly_reporting)
This article will walkthrough how to view multiple hourly data values.

### [Make an Hourly Claim](https://mrets.github.io/Help/hourly_data_make_an_hourly_claim)
This article will walkthrough how to make an hourly claim in M-RETS.

<br>

## Accounts

### [Add Active, Reserve, or Retirement Account](https://mrets.github.io/Help/add_account)
This article will walkthrough how to create an active or retirement account(s).

### [Reserve Accounts](https://github.com/mrets/Help/blob/master/Reserve_Account_Overview.md)
This article will provide an overview of reserve accounts. 

<br>

## Projects

### [Distributed Generation Groups(DGGs)](https://mrets.github.io/Help/distributed_generation_group)
This article will walkthrough how to register a DGG. 

### [Register a New Generator](https://mrets.github.io/Help/projects_new_generator)
This article will walkthrough how to register a new generator.

### [Transferring or Reassigning Generators/Projects](https://mrets.github.io/Help/projects_transferring_generator)
This article will walkthrough how to transfer or reassign a generator.


<br>

## Generation

### [Uploading Generation Data](https://mrets.github.io/Help/generation_upload_data)
This article will walkthrough how to upload generation data.

### [Multi-Fuel Issuance: Entering Fuel Splits for Multi-Fuel Generators](https://mrets.github.io/Help/generation_multi_fuel)
This article will walkthrough how to input a fuel splut for multi fuel generators. 

### [Feasibility Failures](https://mrets.github.io/Help/generation_feasibility_failures)
This article will walkthrough what needs to be done when your generation enters a pending: capacity feasibility status. 

### [Generation Upload Data Format](https://mrets.github.io/Help/generation_upload_format)
This article will walkthrough the generation upload data format.

### [Generation Adjustments](https://mrets.github.io/Help/generation_adjustments)
This article will walkthrough how to upload a generation adjustment. 

### [MISO Generation Reporting](https://mrets.github.io/Help/MISO)
This article will walkthrough how to use MISO to report generation data. 

### [SPP Generation Reporting]()
This article will walkthrough how to use SPP to report generation data. 

<br>

## Programs

### [Adding a Program](https://mrets.github.io/Help/program_adding_program)
This article will describe the basics of Programs

### [Participating in a Program](https://mrets.github.io/Help/program_participating_programs)
A Guide to Participating in Compliance or Voluntary Programs

<br>

<br>

## Billing, Invoices and Organization Management

### [Adding a New User to an Organization](https://mrets.github.io/Help/billing_adding_new_user)
This article will help you figure out how to add a new user to your organization

### [Removing a User from your Organization](https://mrets.github.io/Help/billing_removing_user)
This article will walk through how to remove a user by setting them to Inactive.

### [Update/Add your Billing Contact Information](https://mrets.github.io/Help/billing_add_update_billing_info)
This article will walkthrough how to update or add a new billing contact to your organization. 

### [Email Notifications](https://mrets.github.io/Help/billing_email_notifications)
This article will walkthrough how to add or remove email notifications for your login. 

### [View the Status of Current and Past Invoices](https://mrets.github.io/Help/billing_viewing_invoices)

### [Paying an Invoice](https://mrets.github.io/Help/paying_invoices)
This article will walkthrough where to find your organizations invoices in M-RETS. 

### [Admin M-RETS Administrator Assisted User Edits](https://mrets.github.io/Help/billing_adding_users_admin_assisted)
This article will walkthrough how to regain access to your organization if you no longer have access. 

<br>

# Compliance
Learn more about how RECs are used for compliance.

### [Illinois Alternative Retail Electric Supplier (ARES) Information](https://mrets.github.io/Help/compliance_il_ares)
IL ARES Information

### [Wisconsin Compliance](https://mrets.github.io/Help/compliance_wi_compliance)
Wisconsin Compliance Assistance - Link to PSC Website

### [Minnesota Compliance Training](https://mrets.github.io/Help/compliance_mn_compliance)
Assistance with Completing Minnesota RPS and Green Pricing Retirements in M-RETS

<br>

# Platform Documents
Terms of Use, Privacy Policy, and other legal documents

### [M-RETS Operating Procedures](https://www.mrets.org/wp-content/uploads/2024/02/MRETS-Operating-Procedure-2024.1.pdf)

### [Terms of Use](https://mrets.github.io/Help/platform_terms_of_use)
Rules for using the M-RETS system
